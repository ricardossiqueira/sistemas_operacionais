...
O PROCESSAMENTO RETORNA UMA OPERACAO COM OS DIGITOS ENVIADOS PELO CLIENTE. CASO UM CLIENTE SE DESCONECTE DURANTE
...
//funcao que faz hash do  pid
unsigned int hash(unsigned int x) {
    return x;
}

//funcao que retorna a hash de todos os digitos presentes no pid do processo cliente
void requestMessageReceived(Message *msg)
{
	//envia 2 que é o código da mensagem, junto com a hash
	sem_wait(&sm_ClientList);
	dprintf(1,"Processando msg do cliente %d\n",msg->id);
	if(checkSocketInClientList(msg->fd))
	{
		strncpy(hello, "2|\0", 1024);
		unsigned int sum = hash(msg->id);
		char soma_str[20];
		sprintf(soma_str,"%d",sum);
...
void *processQueue()
{
	...
	while(KEEP_READING_QUEUE)
	{
		...
		if(!vazia)
		{
			sem_wait(&sm_Queue);
			msg = fila_retira(Q);
			dprintf(1,"Retirada da fila a msg do pid %d e fd %ld\n",msg->id,msg->fd);
			sem_post(&sm_Queue);
			switch(msg->type)
			{
				case 1:
